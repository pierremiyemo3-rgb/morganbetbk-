...
/* ═══════════════════════ SETTINGS ═══════════════════════ */
function Settings({bankroll,onUpdateBankroll,onResetRequest,onChangePin,cur,onCurChange,showToast,t}){
...
export default function App(){
  ...
  useEffect(()=>{ loadData().then(d=>{ if(d){setData(d);if(d.bankroll?.setup)setPage("dashboard");if(d.lang)setLang(d.lang);if(d.cur)setCur(d.cur);} setLoading(false); }); },[]);
  ...
  const handleUpdateBankroll=(eur,mode)=>{ let n=data.bankroll.eur; if(mode==="set")n=eur;else if(mode==="add")n+=eur;else n=Math.max(0,n-eur); persist({...data,bankroll:{...data.bankroll,eur:n}}); };
  ...
  if(page==="landing")return <Landing t={t} lang={lang} setLang={handleLangChange} onEnter={()=>{ if(data?.bankroll?.setup)setPage("dashboard");else setPage("setup"); }}/>;
  if(page==="setup")return <Setup t={t} onSave={handleSetup}/>;

  const bankroll=data?.bankroll?.eur||0, bets=data?.bets||[];
  const NAV=[{id:"dashboard",icon:"🏠",label:t.navD},{id:"history",icon:"📁",label:t.navH},{id:"stats",icon:"📊",label:t.navS},{id:"bilan",icon:"📅",label:t.navB},{id:"newbet",icon:"➕",label:t.navN,admin:true},{id:"settings",icon:"⚙️",label:t.navSet,admin:true}];
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div className="mob-top" style={{position:"sticky",top:0,zIndex:90,background:"rgba(6,14,26,.97)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${C.border}`,padding:"10px 14px",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{color:C.gold,fontWeight:800,fontSize:13}}>{cur==="XAF"?fXaf(bankroll):fEur(bankroll)}</div>
          </div>
          {isAdmin?<button onClick={()=>setIsAdmin(false)} style={{background:"rgba(139,92,246,.15)",border:"1px solid rgba(139,92,246,.3)",borderRadius:20,padding:"3px 8px",color:"#a78bfa",fontSize:10,fontWeight:700,cursor:"pointer"}}>🔐</button>:<button onClick={()=>setShowPin(true)} style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:20,padding:"3px 8px",color:C.muted,fontSize:10,cursor:"pointer"}}>🔒</button>}
        </div>
      </div>
      ...
      <main style={{paddingBottom:80}}>
        ...
        {page==="settings"&&(isAdmin?<Settings bankroll={bankroll} onUpdateBankroll={handleUpdateBankroll} onResetRequest={handleResetRequest} onChangePin={p=>persist({...data,pin:p})} cur={cur} onCurChange={handleCurChange} showToast={showToast} t={t}/>:<AccessDenied/>)}
      </main>
      ...
      <nav className="bot-nav" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:90,background:"rgba(6,14,26,.97)",backdropFilter:"blur(16px)",borderTop:`1px solid ${C.border}`,display:"none",paddingBottom:"env(safe-area-inset-bottom,0)"}}>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>navTo(n.id)} style={{flex:1,padding:"9px 2px 7px",background:"none",border:"none",color:page===n.id?C.gold:n.admin?(isAdmin?"#a78bfa":C.muted):C.muted,cursor:"pointer",fontFamily:"inherit",display:"flex",flexDirection:"column",alignItems:"center",gap:2,minWidth:0}}>
            <span style={{fontSize:18,lineHeight:1}}>{n.icon}</span>
            <span style={{fontSize:"clamp(7px,2vw,10px)",fontWeight:page===n.id?700:400,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%"}}>{n.label}{n.admin&&!isAdmin?"🔒":""}</span>
          </button>
        ))}
      </nav>
    </div>
